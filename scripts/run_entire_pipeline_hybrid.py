...
def align_fastq_to_bam_piped(
    ...
    try:
        ...
        # Check return codes - BUT ignore minimap2 warnings
        # Minimap2 often returns non-zero even on success
        if p3.returncode != 0:
            print(f"ERROR: BAM sorting failed: {stderr3.decode('utf-8')[:500]}")
            return False
        
        # Check if BAM was created (better indicator of success)
        ...
        # ADD MD TAG (CRITICAL FOR DRM ANALYSIS)
        print("INFO: Adding MD tags (required for mutation detection)...")
        temp_bam_no_md = output_bam.replace('.bam', '.temp.bam')
        
        # Rename current BAM to temp
        os.rename(output_bam, temp_bam_no_md)
        
        # Add MD tag
        calmd_cmd = ['samtools', 'calmd', '-b', temp_bam_no_md, ref_fasta]
        with open(output_bam, 'wb') as bam_out:
            result = subprocess.run(calmd_cmd, stdout=bam_out, stderr=subprocess.PIPE)
        
        if result.returncode != 0:
            print(f"ERROR: Adding MD tag failed: {result.stderr.decode('utf-8')}")
            return False
        
        # Remove temp file
        os.remove(temp_bam_no_md)
        print("INFO: MD tags added successfully.")
