...
# These are the assumed coordinates we are testing.
...
def run_diagnostics(read, ref_sequences):
    """
    Finds and prints detailed information for any mutation within the RT gene.
    """
    ...
    for read_pos, ref_pos, ref_base in aligned_pairs:
        if read_pos is not None and ref_pos is not None and ref_base is not None:
            ...
            if read_base.upper() != ref_base.upper():
                ...
                # --- DIAGNOSTIC LOGIC ---
                # If the mutation is in the RT gene, print a detailed report.
...
def main():
    parser = argparse.ArgumentParser(description="Diagnostic tool for the HIV DRM pipeline.")
    ...
    print("--- Running HIV DRM Pipeline in Diagnostic Mode ---")
